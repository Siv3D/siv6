...
namespace s3d
{
	...
	{
		[[nodiscard]]
		inline constexpr AdapterVendor ToAdapterVendor(const uint32 vendorID) noexcept
		{
			switch (vendorID)
			{
			case 0x10DE:
		...
		[[nodiscard]]
		inline constexpr StringView ToString(const D3D_FEATURE_LEVEL featureLevel) noexcept
		{
			switch (featureLevel)
			{
		...
		[[nodiscard]]
		static size_t ChooseAdapterIndex(const Array<D3D11Adapter>& adapters, const bool favorIntegrated)
		{
			if (favorIntegrated)
			{
		...
		[[nodiscard]]
		static Optional<D3D11DeviceInfo> CreateDeviceHardware(
		...
		[[nodiscard]]
		static Optional<D3D11DeviceInfo> CreateDeviceWARP(
			PFN_D3D11_CREATE_DEVICE pD3D11CreateDevice,
			const uint32 deviceFlags,
		...
		[[nodiscard]]
		static Optional<D3D11DeviceInfo> CreateDeviceReference(
			PFN_D3D11_CREATE_DEVICE pD3D11CreateDevice,
			const uint32 deviceFlags,
		...
		[[nodiscard]]
		static Optional<D3D11DeviceInfo> CreateDevice(
			PFN_D3D11_CREATE_DEVICE pD3D11CreateDevice,
	...
	D3D11Device::D3D11Device()
	{
		...
		{
			m_referenceFeatureLevel = detail::GetREFFeatureLevel(pD3D11CreateDevice);
